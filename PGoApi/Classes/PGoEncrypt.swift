...
public class PGoEncrypt {
    ...
    public func randomBytes() -> NSData {
        var randomBytes = [UInt8](count: 32, repeatedValue: 0)
        SecRandomCopyBytes(kSecRandomDefault, 32, &randomBytes)
        return NSData(bytes: randomBytes, length: 32)
    }
    
    public func encryptUsingLib(input: NSData) -> NSData {
        let iv = randomBytes()
